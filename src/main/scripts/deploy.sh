...
docker run -dit --name potic-articles --link potic-pocket-api --link potic-mongodb --ling potic-users -e LOG_PATH=/logs -v /logs:/logs -e MONGO_PASSWORD=$MONGO_PASSWORD -p 40402:8080 potic/potic-articles:$TAG_TO_DEPLOY
